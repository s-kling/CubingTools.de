...
function setupCompetition() {
  ...
  if (competitionName && maxCompetitors) {
    ...
    events.forEach((event) => {
      ...
      else if (event.id === "333_oh_bld_team_relay")
        newTitle = "Relays Blindfolded";
      ...
      if (newTitle && newTitle !== currentTitle) {
        ...
        titleDiv.addEventListener("click", () => {
          ...
          while (
            nextElement &&
            !nextElement.classList.contains("event-section-title")
          ) {
            const checkbox = nextElement.querySelector(
              'input[type="checkbox"]'
            );
            if (checkbox) {
...
function handleAddingEventDivs(event, eventCheckboxes) {
  if (!eventCheckboxes)
    eventCheckboxes = document.getElementById("event-checkboxes");
  ...
  checkbox.addEventListener("click", () => {
    ...
    if (checkbox.classList.contains("checked")) {
      ...
      dynamicGroupCheckbox.addEventListener("click", (e) =>
        e.stopPropagation()
      );
      ...
      dynamicGroupCheckbox.addEventListener("change", () => {
        groupInput.style.display = dynamicGroupCheckbox.checked
          ? "none"
          : "block";
      });
...
async function generateGroups() {
  ...
  for (const eventId of selectedEvents) {
    ...
    const dynamicGroupCheckbox = document.getElementById(
      `dynamic-group-${eventId}`
    );
    let numGroups;
    ...
    if (dynamicGroupCheckbox && !dynamicGroupCheckbox.checked) {
      ...
      numGroups =
        parseInt(groupInput.value, 10) < eventCompetitors.length
          ? parseInt(groupInput.value, 10)
          : eventCompetitors.length;
    } else {
    ...
    for (let groupIndex = 0; groupIndex < eventGroups.length; groupIndex++) {
      ...
      const topCompetitors = eventCompetitors.slice(
        0,
        numOfHelpers * eventGroups.length
      );
      const availableScramblers = topCompetitors.filter(
        (competitor) => competitor.groupAssignments[eventId] !== groupIndex + 1
      );

      if (availableScramblers.length > 0 || eventCompetitors.length == 1) {
        const scramblers = [];
        for (let i = 0; i < numOfHelpers; i++) {
          const totalWeight = availableScramblers.reduce(
        (sum, competitor, index) => {
          const weight = competitor.wcaId
            ? availableScramblers.length - index
            : 1;
          return sum + weight;
        },
        0
        ...
        const weight = availableScramblers[j].wcaId
          ? availableScramblers.length - j
          : 1;
        randomWeight -= weight;
        if (randomWeight <= 0) {
        ...
        // If there is only one competitor, assign another random competitor as a scrambler
        if (eventCompetitors.length == 1) {
          const randomCompetitor = shuffledCompetitors.find(
        (competitor) => competitor.id !== eventCompetitors[0].id
          );
          group.scramblers = [randomCompetitor];
        } else {
    ...
    .forEach((competitor) => {
      ...
      selectedEvents.forEach((eventId) => {
        ...
        const judgeGroups = competitionData.groups[eventId]
          .map((group, index) =>
            group.judges.includes(competitor) ? index + 1 : null
          )
        ...
        const runGroups = competitionData.groups[eventId]
          .map((group, index) =>
            group.runners.includes(competitor) ? index + 1 : null
          )
          .filter((group) => group !== null)
          .join(", ");
