document.addEventListener("DOMContentLoaded", function () {
  const openDate = new Date("2025-01-11T17:00:00Z");
  ...
  document.getElementById("registration-open-date").textContent =
    userTimeZoneDate.toLocaleString("en-GB", {
      day: "numeric",
      month: "long",
      year: "numeric",
      hour: "2-digit",
  ...
  eventCheckboxes.forEach((checkbox) => {
    checkbox.addEventListener("click", () => {
      ...
      if (checkbox.classList.contains("checked")) {
        ...
        checkbox.innerHTML = `<p>${checkbox.innerText.replace(
          " (Selected)",
          ""
        )}</p>`;
      }
...
document
  ...
  .addEventListener("submit", async function (event) {
    ...
    if (response.ok) {
      ...
      alert(
        "Registration successful! Please check your email for a confirmation message."
      );
    } else {
