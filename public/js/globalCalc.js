...
function populateSessionDropdown() {
  ...
  for (const sessionKey in sessions) {
    if (sessions[sessionKey].length > 0 && sessionKey !== "properties") {
      ...
      option.textContent =
        sessionNames[sessionId]?.name || `Session ${sessionId}`;
...
function generateChartFromSession() {
  ...
  originalArray = sessionData
    .map((item) =>
      Math.floor(
        item[0][0] < 0 ? Infinity : Math.floor(item[0][0] + item[0][1]) / 100
      )
    )
...
function updateChart() {
  ...
  const labels = Object.keys(frequencyMap).map((time) =>
    formatTimeForChart(time * 1000)
  ); // Convert back to ms
  ...
  topTimes.forEach(([num, freq]) => {
    ...
    listItem.textContent = `Time ${formatTimeForTop5(
      num * 1000
    )}: ${freq} times`;
